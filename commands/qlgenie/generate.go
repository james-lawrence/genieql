...
func (t *generateCrud) Execute(*kingpin.ParseContext) error {
	...
	scanner := genieql.Scanner{Name: "CrudScanner"}.Build(columnMap, ssoIdentity)
	imports := genieql.NewImports().Imports("database/sql", "bitbucket.org/jatona/sso").Decl()

	fset := token.NewFileSet()

	fmt.Fprintf(buffer, "package %s\n\n", "sso")

	if err := format.Node(buffer, fset, imports); err != nil {
		log.Fatalln(err)
	}
	fmt.Fprintf(buffer, "\n\n")

	if err := format.Node(buffer, fset, scanner); err != nil {
		log.Fatalln(err)
	}

	fmt.Fprintf(buffer, "\n\n")
	gen := genieql.NewCRUDWriter(
		buffer,
		typName,
		t.table,
		naturalkey,
	...
	if err := gen.Write(genieql.PostgresqlCRUDGen, fset); err != nil {
		log.Fatalln(err)
	}
...
// command: genieql %s
// DO NOT MODIFY`, strings.Join(os.Args[1:], " "))
	printer.Fprint(os.Stdout, fset, file)
	fmt.Fprintf(os.Stdout, "\n")
	printer.Fprint(os.Stdout, fset, imports)
	fmt.Fprintf(os.Stdout, "\n\n")
	printer.Fprint(os.Stdout, fset, scanner)
	return nil
}
