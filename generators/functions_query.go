...
func NewQueryFunction(ctx Context, options ...QueryFunctionOption) genieql.Generator {
	...
	if qf.QueryerFunction != nil {
		// TODO: clean this up.
		if astutil.TypePattern(astutil.ExprTemplateList("*sql.Row", "error")...)(pattern...) {
			qf.NilErr = ast.NewIdent("nil")
		}
	} else if queryPattern(pattern...) {
		qf.QueryerFunction = ast.NewIdent("Query")
	} else if queryRowPattern(pattern...) || queryRowPatternAlternate(pattern...) {
...
var queryRowPatternAlternate = astutil.TypePattern(astutil.ExprTemplateList("*sql.Row", "error")...)
...
func defaultQueryFuncTemplate(ctx Context) *template.Template {
	const defaultQueryFunc = `// {{.Name}} generated by genieql
		func {{.Name}}({{ .Parameters | arguments }}) {{ .ScannerType | expr }} {
			...
			{{ end }}
			{{ end }}
			return {{ .ScannerFunc | expr }}({{ .Queryer | expr }}{{ if .NilErr }},{{ .NilErr| expr}}{{end}})
		}
	`
	var (
