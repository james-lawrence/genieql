		...
		It("should be able to scan a result", func() {
			for _, testEntry := range table {
				func() {
					...
					defer tx.Rollback()
					scanner := NewType1StaticScannerStaticRow(tx.QueryRow(Type1Insert, explode(testEntry)...))
		...
		It("should be able to scan a result", func() {
			for _, testEntry := range table {
				func() {
					...
					scanner := NewType1StaticScannerStatic(tx.Query(Type1Insert, explode(testEntry)...))
		...
		It("should ignore unmapped fields", func() {
			...
			for _, testEntry := range table {
				func() {
					...
					scanner = NewType1DynamicScannerDynamic(tx.Query(q))
					Expect(scanner.Next()).To(BeTrue())
					Expect(scanner.Scan(&found)).ToNot(HaveOccurred())
					Expect(scanner.Close()).ToNot(HaveOccurred())
